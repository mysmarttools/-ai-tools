...
function createImage(){
    ...
    const styleEl = document.getElementById("designStyle");
    const canvas = document.getElementById("canvas");

    if(!textEl || !styleEl || !canvas) return;
    ...
    if(style === "sale"){
        ...
        textColor = "#2f3542"; // Black/Gray text for white background
    }
    ...
    // Dynamic Text wrapping (Helper to avoid text overflow)
    wrapText(ctx, text, 400, 200, 700, 50);
}
...
function checkLength(){
    ...
    // Run only if elements actually exist on the page
